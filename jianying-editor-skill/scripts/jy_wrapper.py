...
class JyProject:
    ...
    def add_transition_simple(self, transition_name: str, duration: str = "0.5s", track_name: str = "VideoTrack", effect_id: str = None):
        ...
        # Transition must be attached to the *previous* segment. Need at least 2 segments.
        if not track or len(track.segments) < 2:
            print(f"⚠️ Cannot add transition: Track '{track_name}' not found or needs >=2 segments.")
            return
        ...
        # NOTE: pyJianYingDraft requires transitions to be added on the *previous* segment.
        prev_seg = track.segments[-2]
        try:
            # 关键修复: 转换 duration 为微秒
            dur_us = tim(duration)
            prev_seg.add_transition(trans_enum, duration=dur_us)
